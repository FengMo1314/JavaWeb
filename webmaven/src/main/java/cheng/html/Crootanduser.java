...
public class Crootanduser extends HttpServlet {
    @Override
...
//                    response.sendRedirect("./chtml/root.jsp");
                break;
            case "2"://修改
                List<TableBeanCheng> listtable = cit.isUpdataByList(usermesgs);
                request.setAttribute("tableList", listtable);
//                    for (UpdataBeanCheng ubc:listupdata) {
//                        cit.isUpdata(ubc.getId(), ubc);
//                    }
                request.getRequestDispatcher("./chtml/updataByroot.jsp").forward(request, response);
                break;
            case "3"://删除
